...
from ..utils.config import AffineCandidateConfig, ScoringConfig, BlobDetectorConfig, GridConfig
...
class ThreeDotDetector:
    def __init__(
        self,
        BLOB_CONFIG: BlobDetectorConfig,
    ...
    def detect_blobs(self, gray: np.ndarray) -> Tuple[np.ndarray, np.ndarray, List[cv2.KeyPoint]]:
        ...
        if (self.blob_config.retrieval in ("external", "list")):
            # binarize: 사용자가 지정하지 않으면 Otsu(THRESH_BINARY), 지정 시 고정 임계
            if self.blob_config.bin_threshold and 0.0 < float(self.blob_config.bin_threshold) < 1.0:
                thr = int(round(255.0 * float(self.blob_config.bin_threshold)))
                _, binarized_image = cv2.threshold(gray, thr, 255, cv2.THRESH_BINARY)
            else:
            ...
            mode = cv2.RETR_EXTERNAL if self.blob_config.retrieval == "external" else cv2.RETR_LIST
            ...
            for cnt in contours:
                ...
                # 필터 1) 최소 면적: 너무 작은 컨투어 제외 (min_area={self.blob_config.min_area})
                if area < float(self.blob_config.min_area):
                    removed_area += 1
                    continue
                ...
                if r <= 0:
                    removed_radius += 1
                    continue
                # 필터 2b) 면적 상한: 먼지/큰 아티팩트 제거 
                if self.blob_config.max_area is not None and area > float(self.blob_config.max_area):
                    removed_area += 1
                    continue
                ...
                if fill_circle < float(self.blob_config.min_fill):
                    removed_fill += 1
                    continue
                ...
                if len(cnt) >= 5:
                    try:
                        ...
                        if a > 0:
                            inertia_ratio = b / a
                            min_inertia = float(np.sqrt(max(0.0, 1.0 - float(self.blob_config.max_eccentricity) ** 2)))
            ...
            removed_fnn = 0
            if len(pts) > 0 and self.blob_config.fnn_filter and self.blob_config.fnn_filter > 1:
                pts0, diam0, kps0 = pts.copy(), diam.copy(), kps[:]
                k = min(max(2, int(self.blob_config.fnn_filter)), len(pts))
                try:
                    tree = KDTree(pts)
                    d4, _ = tree.query(pts, k=min(int(max(1, self.blob_config.fnn_local_k)), len(pts)))
                    local_d = float(np.median(d4[:, 1])) if len(pts) >= 2 else 1.0
                    R = float(max(0.0, self.blob_config.fnn_radius_beta) * local_d)
                    dk, _ = tree.query(pts, k=k)    
                    required_neighbors = max(1, min(int(self.blob_config.fnn_filter) - 1, k - 1))
                    ...
                    # 과도 필터링 시 롤백
                    min_kept_abs = int(max(0, self.blob_config.fnn_rollback_min_kept))
                    min_kept_rel = int(max(1, round(len(pts0) * float(self.blob_config.fnn_rollback_fraction))))
            ...
            def _create_blob_detector() -> cv2.SimpleBlobDetector:
                ...
                p.minConvexity = float(max(0.0, min(1.0, self.blob_config.min_fill)))
                e = float(max(0.0, min(0.9999, self.blob_config.max_eccentricity)))
                min_inertia = float(np.sqrt(max(1e-6, 1.0 - e * e)))
                p.filterByInertia = True
                p.minInertiaRatio = min_inertia

                p.minDistBetweenBlobs = float(self.blob_config.sbd_min_dist_between_blobs)                
                if self.blob_config.sbd_blob_color is not None:
                    p.filterByColor = True               
                    p.blobColor = int(self.blob_config.sbd_blob_color)                
                # repeatability 요구 사항을 충족하도록 threshold 밴드를 구성
                if self.blob_config.sbd_fixed_threshold is not None:
                    if self.blob_config.sbd_fixed_threshold == 'otsu':
                        ret, _ = cv2.threshold(gray, 0, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU)
                        thr0 = float(ret)
                    else:
                        thr0 = float(self.blob_config.sbd_fixed_threshold)
                    thr0 = max(1.0, min(254.0, thr0))
                    # 밴드 폭과 스텝은 설정에서 가져옴
                    half = max(0.5, float(self.blob_config.sbd_band_halfwidth))
                    step = max(0.5, float(self.blob_config.sbd_threshold_step))
                ...
                p.minRepeatability = int(max(1, int(self.blob_config.sbd_min_repeatability)))
            ...
            pts = np.array([k.pt for k in kps], dtype=np.float32) if kps else np.zeros((0, 2), np.float32)
            diam = np.array([k.size for k in kps], dtype=np.float32) if kps else np.zeros((0,), np.float32)
            self._last_binarized_image = None
        
            # Optional neighborhood density filter
            if len(pts) > 0 and self.blob_config.fnn_filter and self.blob_config.fnn_filter > 1:
                pts0, diam0, kps0 = pts, diam, kps
                k = min(max(2, self.blob_config.fnn_filter), len(pts))
                tree = KDTree(pts)
                d4, _ = tree.query(pts, k=min(5, len(pts)))
                local_d = float(np.median(d4[:, 1])) if len(pts) >= 2 else 1.0
                R = float(10.0 * local_d)
                dk, _ = tree.query(pts, k=k)
                required_neighbors = max(1, min(self.blob_config.fnn_filter - 1, k - 1))
                valid = (dk[:, 1:] <= R).sum(axis=1) >= required_neighbors
                pts = pts[valid]
                diam = diam[valid]
                kps = [kp for kp, ok in zip(kps, valid) if ok]
            ...
            get_logger().error('[FAIL] Invalid retrieval option: %s', self.blob_config.retrieval)
            raise ValueError(f"Invalid retrieval option: {self.blob_config.retrieval}")
        return pts, diam, kps
    ...
    def find_central_candidates(self, pts: np.ndarray, order: np.ndarray, ratio: np.ndarray, diam: np.ndarray) -> List[CentralCandidate]:
        ...
        k24 = max(2, self.candidate_config.neighbors_central)
    ...
    @staticmethod
    def _score_affine(A: np.ndarray, pts_centered: np.ndarray, thr: float) -> Tuple[Tuple[float, int], np.ndarray]:
    ...
    def choose_Tc(self, pts: np.ndarray, cands: List[CentralCandidate]) -> Tuple[        
            np.ndarray,                       # Tc: 최적 아핀 행렬 (2x3)
            int,                              # 중심점 인덱스
            Tuple[int, int, int],             # triplet (center, plus, minus)
            np.ndarray,                       # 맵핑된 점 P
            np.ndarray,                       # 24-NN 인덱스
            Tuple[float, int],                # (mean_error, hit 수)
            int                               # 평가한 후보 총 개수
        ]:
        ...
        for cand in cands:
            ...
            for A in self._affines_for_triplet(Pc, Pp, Pm):
                score, P = self._score_affine(A, pts, thr=self.score_config.affine_score_thr)
    ...
    @staticmethod
    def grid_assign(P: np.ndarray, thr: float, fnn_grid_explore_size:int)-> Dict[Tuple[int, int], int]:
        ...
        get_logger().debug('[DEBUG] Grid assign: center_idx=%d, center_key=%s, center_dist=%.3f, threshold=%.3f', cidx, str(center_key), center_dist, thr)
        ...
        while frontier:
            ...
            for du in (-1, 0, 1):
                for dv in (-1, 0, 1):
                    ...
                    if key in assigned:
                        continue
                    if abs(key[0]) > fnn_grid_explore_size or abs(key[1]) > fnn_grid_explore_size:
                        continue
    ...
    @staticmethod
    def dilate_with_local_refine(
        pts: np.ndarray,
        base_transform: np.ndarray,
        initial_grid: Dict[Tuple[int, int], int],
        thr: float,
        max_grid_size: float,
        min_seed_pairs: float,
        sigma_prune: float,
        estimatation_affine_method_opencv=cv2.LMEDS
    ) -> Dict[Tuple[int, int], int]: # return: grid_assign: Dict[Tuple[int, int], int]
        """Dilate gridded area (Step 6) using local 2x2 squares to refine transform.

        단위 주의: thr는 격자 공간 단위. 픽셀(px) 아님.
        Algorithm:
                ...
                for du in (-1, 0, 1):
                    for dv in (-1, 0, 1):
                        ...
                        if key in uv_assigned:
                            continue
                        if abs(key[0]) > max_grid_size or abs(key[1]) > max_grid_size:
                            continue
        ...
        while grew_total:
            ...
            for sq in list_squares(keys):
                ...
                T_next, _ = cv2.estimateAffine2D(src, dst, method=estimatation_affine_method_opencv) # cv2.LMEDS or cv2.RANSAC both are acceptable
                if T_next is None:
                    explored.add(sq)
                    continue
    ...
    def run_on_image(self, image_path: Path, gray: np.ndarray) -> DetectionResult:
        ...
        grid = self.dilate_with_local_refine(
            pts,
            Tc,
            initial_grid=grid,
            thr=self.score_config.dilation_thr,
            max_grid_size=self.grid_config.max_grid_size,
            min_seed_pairs=self.score_config.min_seed_pairs,
...
def example(debug_dir, image_path):
    """단일 이미지 디버그 실행 예제."""
    from ..utils.config import AffineCandidateConfig, ScoringConfig, BlobDetectorConfig, GridConfig
    BLOB_CONFIG = BlobDetectorConfig() # 시편 의존성 큼, 파라미터 설정 필요
    GRID_CONFIG = GridConfig() # 시편 의존성 큼, 파라미터 설정 필요
    AFFINE_CANDIDATE_CONFIG = AffineCandidateConfig() # 시편 의존성 거의 없음
    SCORE_CONFIG = ScoringConfig() # 시편 의존성 거의 없음

    det = ThreeDotDetector(BLOB_CONFIG, GRID_CONFIG, AFFINE_CANDIDATE_CONFIG, SCORE_CONFIG, Path(debug_dir))
